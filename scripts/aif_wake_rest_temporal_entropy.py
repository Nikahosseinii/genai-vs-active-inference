# ==============================================================
# Wake-only vs Wake + Rest — Temporal entropy H_T
#
# Logs:
#   - Spatial entropy  H_S (optional, computed but not plotted here)
#   - Temporal entropy H_T (inter-event gap entropy)
#
# Intended for Figure 9(b)-style temporal entropy panel (or related).
#
# Usage:
#   python scripts/aif_wake_rest_temporal_entropy.py
#   python scripts/aif_wake_rest_temporal_entropy.py --use_x0 --pt_path data/x0.pt
# ==============================================================
import argparse
import random
...
# ────────────────────────────────────────────────────────────────
# Helpers: posterior proxy + entropy measures
# ────────────────────────────────────────────────────────────────
def posterior_Q(g, D):
...
def spatial_entropy(g: nx.Graph) -> float:
    """Path-length entropy (natural log)."""
...
# ────────────────────────────────────────────────────────────────
# ACTIVE-INFERENCE SCHEDULER (logs H_S and H_T)
# ────────────────────────────────────────────────────────────────
def run_schedule(
    G0,
    *,
    ...
    for t in range(steps):
        ...
        # ---- rest: entropy-max replay proxy (random add/remove/swap)
        else:
    ...
    return np.array(H_S), np.array(H_T)
...
def main():
    ...
    ap.add_argument("--start_rest_wakeonly", type=int, default=2000, help=">=steps means never rests")
    ap.add_argument("--start_rest_wakerest", type=int, default=0, help="Rest starts at this step for wake+rest")
    ...
    ap.add_argument("--out_name", type=str, default="temporal_entropy_wake_vs_rest.png")
    args = ap.parse_args()
    ...
    # Run two conditions
    _, HT_wake = run_schedule(
        G_seed,
        steps=args.steps,
        start_rest=args.start_rest_wakeonly,
    ...
    _, HT_rest = run_schedule(
        G_seed,
        steps=args.steps,
        start_rest=args.start_rest_wakerest,
        rest_every=args.rest_every,
        rest_replays=args.rest_replays,
        add_edges=args.add_edges,
        p_event=args.p_event,
    ...
    # Plot temporal entropy only
    plt.figure(figsize=(6, 4))
    plt.plot(HT_wake, "r--", label="Wake only")
    plt.plot(HT_rest, "b", label="Wake + Rest")
    plt.xlabel("Network Rewiring Steps", fontsize=11)
    plt.ylabel("Temporal entropy", fontsize=11)
